...
date: "Slides em: [bit.ly/2DpELlX](bit.ly/2DpELlX)"
output:
...
benchmark <- function(){
  ...
  return(tibble::tibble(For = forloop, Apply = applyloop, Map = maploop))
}

library(furrr)
plan(multisession)

bench_loop <- future_map_dfr(1:10000, ~benchmark())
```
...
bench_loop %>% gather(Metodo, Tempo) %>% 
  ...
  theme_bw() + scale_y_continuous(limits = c(0.30, 0.70)) +
...
É o método mais utilizado pela simplicidade de aplicação.
...
Bootstrap Residual apresentou amplitudes menores do que o Normal:
...
**Esta variação depende da qualidade do ajuste da rega de predição.**
...
De forma geral:
  ...
  - Obter uma amostra $\mathbf{w}$ da Dirichlet;
