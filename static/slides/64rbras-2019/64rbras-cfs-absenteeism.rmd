...
write_xaringan_theme(title_slide_background_color = "white",
                     title_slide_background_image = "Logo2.png", 
...
CFS affects children's life quality **as much as Asthma and Type I Diabetes Melitus** (Kennedy et al., 2010).

--

--- 

First definined by the USA Center for Disease Control  (Holmes et al., 1988).
...
- With at least 4 out of:
  ...
  - Tender lymph nodes;
  - Unrefreshing sleep;
  - Post-extersional malaise*****.

.footnote[[*****] Critisized by not demanding this important symptom (Williams et al., 2014)].
...
Epidemiological studies estimate:

- 410 cases per 100.000 Americans (Jason et al., 1999);
  - Suggests 1 milion cases in EUA alone;
...
CFS, therefore, interfeers in the individual's:
...
Affecting negatively both health and economy.
...
- **Metabolic Syndrome** (Maloney et al. (2010).
  ...
  - Includes high levels of glicosis and abnormal cholesterol.

<br>

- Elevated risk after 10 years of **Giarda infection** (Litleskare et al., 2018).
...
- **Nayve Bayes** obtained **AUC of 0.7** on gene expression data (Huang et al., 2007).
<br><br><br>
...
<br><br><br>
- By **decision trees** on medical questionnaire data, Bronikowski et al. (2011) obtained **accuracy of 71.88%**.
...
The individuals worked on **alternated shifts**: 6hrs shifts with 12hrs of rest.
...
|:---------------------:|:-----------------------:|:---------------:|------------:|-----------:|---------:|
|       Absenteísmo     |     Company regiter     | Binary (Y = 1) |    0.2238   |     -      |   0.00   |
|          Sexo         |      Medical files      | Binary (M = 1) |    0.9646   |     -      |   0.00   |
|         Idade         |      Medical files      |   Discrete     |   36.7504   |   7.1336   |   0.00   |
|          BMI          |       Weight/Height²    |   Continuous   |  174.1032   |   7.3123   |   5.25   |
| Waist/Hips Ratio (n=3)|       Metric tape       |  Continuous    |    0.8566   |   0.1220   |   2.64   |
|  Total Body Fatness   |       Bioimpedance      |  Continuous    |   24.3923   |   8.1730   |   3.50   |
|    Visceral Fatness   |       Bioimpedance      |  Continuous    |    7.6712   |   3.4312   |   3.33   |
...
The minning industry is characterized by:
...
According to Colquhoun e Senn (2000), shift workers contantly complain of:
...
Shift work on mines may leado not only to CFS, but also:
...
The Logistic model was used in 3 approaches:

  - **Stepwise**: variable selection;
  
  - **Elastic-Net** and **Lasso**: variable selection and shrinkage.
...
<br>
The used metric was the **Area Under the ROC curve (AUC)**: Values close to 1 indicate better class discrimination, as oposed to values close to 0.5.
...
The metrics of **Accuracy**, **Sensiticity** and **Specificity** were assessed by the **Bootstrap 632** (Efron and Tibshirani, 1997):
...
The mainly used packages were:
  ...
  - `caret`: Model fitting and seletion;
...
```{r, echo=FALSE, out.height=300, out.width=1000, fig.align='center'}
knitr::include_graphics("CoefTable.png")
```

- Cholesterol and Sodium significative in all three models.
...
```{r, echo=FALSE, out.height=450, out.width=600, fig.align='center'}
knitr::include_graphics("BootEffects1.png")
```
...
```{r, echo=FALSE, out.height=450, out.width=600, fig.align='center'}
knitr::include_graphics("BootEffects2.png")
```
...
```{r, echo=FALSE, out.height=450, out.width=600, fig.align='center'}
knitr::include_graphics("BootEffects3.png")
```
...
```{r, echo=FALSE, out.height=430, out.width=600, fig.align='center'}
knitr::include_graphics("PerfModels.png")
```
...
Models did not discriminate very well.
  - Possibly due to noise of dependent's variable (a binary of a whole year).
...
More detailed studies are needed, i.e., logitudinal studies directly measuring CFS.
...
<br>
```{r echo=FALSE}
knitr::include_graphics("institutions.png")
```
...
Site: [aneisse.com](https://aneisse.com/)<br>
